...
- [x] **12. Path traversal risk** — `spinnerFile` is set by the app author, not user input. No real attack surface; marked non-issue. — `View.py:55` joins `spinnerFile` to `APP_PATH` without validating the path stays within the directory.
